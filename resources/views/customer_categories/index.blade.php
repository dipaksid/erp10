                            ...
                            <td class="text-center col-3">
                                <div class="d-flex align-items-center">
                                    @can('VIEW CUSTOMER CATEGORY')
                                        <a href="{{ action('App\Http\Controllers\CustomerCategoriesController@show',$rcategory->id)}}?searchvalue={{((isset($searchValue))?$searchValue:'')}}&page={{((isset($page))?$page:'') }}" class="btn btn-primary btn-sm mr-1">View</a>
                                    @endcan

                                    @can('EDIT CUSTOMER CATEGORY')
                                        <a href="{{ action('App\Http\Controllers\CustomerCategoriesController@edit',$rcategory->id)}}?searchvalue={{((isset($searchValue))?$searchValue:'')}}&page={{((isset($page))?$page:'') }}" class="btn btn-primary btn-sm mr-1">Edit</a>
                                    @endcan
                                    ...
                                    @can('DELETE CUSTOMER CATEGORY')
                                        <form action="{{ action('App\Http\Controllers\CustomerCategoriesController@destroy', $rcategory->id) }}" method="post" id="deleteForm_{{ $rcategory->id }}">
                                            ...
                                            <button class="btn btn-danger btn-sm mr-1" type="submit" onclick="showConfirmDeleteModal(event, {{ $rcategory->id }})">
                                                Delete
                                            </button>
                                        </form>
                                    @endcan
                                    ...
                                    @can('UPLOAD SYSTEM FILE')
                                        <a href="{{ route('customercategory.uploadsystem', $rcategory->id)}}?searchvalue={{((isset($searchValue))?$searchValue:'')}}&page={{((isset($page))?$page:'') }}" class="btn btn-primary btn-sm">
                                            Upload System File
                                        </a>
                                    @endcan
                                </div>
